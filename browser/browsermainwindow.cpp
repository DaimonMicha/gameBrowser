...
void BrowserMainWindow::setupMenu()
{
    ...
    helpMenu->addAction(tr("About &Daimon gameBrowser"), this, SLOT(slotAboutApplication()));
}
...
{
    statusBar()->showMessage(string, 3500);
}

void BrowserMainWindow::slotUpdateWindowTitle(const QString &title)
{
    if (title.isEmpty()) {
        setWindowTitle(tr("Daimon gameBrowser"));
    } else {
#if defined(Q_WS_MAC)
        setWindowTitle(title);
#else
        setWindowTitle(tr("%1 - Daimon gameBrowser", "Page title and Browser name").arg(title));
#endif
    }
}
...
void BrowserMainWindow::slotHome()
{
    ...
    QString home = settings.value(QLatin1String("home"), QLatin1String("http://daimonmicha.bplaced.net/")).toString();
    loadPage(home);
}
